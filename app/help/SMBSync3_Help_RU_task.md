...
### Поменяйте источник и место назначения
...
### Выберите подкаталоги для синхронизации.
...
### Выполнять задачу синхронизации только при зарядке.
...
### Подтвердите перед переопределением копирования или удаления.
...
- Запускать последующие задачи, если сетевые опции приводят к ошибкам.  
...
**Попробуйте, если при записи в папку PC/NAS вы получите ошибку "Доступ запрещен".**

Когда эта опция установлена, буфер ввода/вывода будет ограничен до 16 КБ для операций записи на SMB хост. 
...
Для следующих синхронизаций можно:

- сравнить только по размеру, или

- можно включить опцию "Не перезаписывать целевой файл, если он более новый, чем исходный файл", чтобы копировать только файлы, измененные позднее ведущим устройством, либо

- можно включить опцию задачи "Получить время последней модификации файлов из пользовательского списка приложения SMBSync3". Однако в настоящее время эта опция недоступна, если целью является SMB. Большинство SMB хостов поддерживают установку последнего измененного времени. 

См. ниже подробную информацию о каждой опции. 
...
### Удалить главный каталог, когда он пуст (только если опция Sync - Move).

При режиме синхронизации Move после перемещения файлов в целевую папку также удаляется главная папка. 
